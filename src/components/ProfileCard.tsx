  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      if (anyMotion?.requestPermission) {
        anyMotion.requestPermission().then((s: string) => { if (s === 'granted') window.addEventListener('deviceorientation', do_); }).catch(console.error);
      } else {
      ...
      <div ref={shellRef} className="pc-card-shell">
        <section className="pc-card">
          <div className="pc-inside">
            ...
            {/* Floating code icons — match the reference */}
            <div className="pc-float-icons" style={{ pointerEvents: 'none' }}>
              {[
                { icon: '</>',  top: '14%',  left: '10%',  size: 22, delay: 0 },
                { icon: '{}',   top: '22%',  right: '12%', size: 18, delay: 0.6 },
                { icon: '</>',  top: '44%',  left: '7%',   size: 16, delay: 1.1 },
                { icon: '()',   top: '38%',  right: '9%',  size: 17, delay: 0.3 },
                { icon: '{}',   top: '62%',  left: '13%',  size: 14, delay: 0.9 },
                { icon: '=>',   top: '58%',  right: '14%', size: 15, delay: 1.5 },
              ].map((ic, i) => (
                <span
                  key={i}
                  className="pc-float-icon"
                  style={{
                    position:   'absolute',
                    top:        ic.top,
                    left:       'left' in ic ? (ic as any).left : undefined,
                    right:      'right' in ic ? (ic as any).right : undefined,
                    fontSize:   `${ic.size}px`,
                ...
                <div className="pc-user-info">
                  <div className="pc-user-details">
                    <div className="pc-mini-avatar">
                      <img src={miniAvatarUrl || avatarUrl} alt={`${name || 'User'} mini`} loading="lazy" />
                    </div>
                  ...
                  <button className="pc-contact-btn" onClick={handleContactClick} type="button" style={{ pointerEvents: 'auto' }}>
                    {contactText}
                  </button>
                </div>
              )}
